...
router.post('/users/signup', async (req, res) => {
    ...
    if(lastname.length <= 0) {
        errors.push({text: 'Por favor escribe tus apellidos'});
    }
    if(email.length <= 0) {
    ...
    if(password != confirm_password) {
        errors.push({text: 'Las contraseñas no coincide'});
    }
    if(errors.length > 0) {
        ...
        if(emailUser) {
            req.flash('error_msg', 'Tu usuario ha sido registado anteriormente');
            res.redirect('/users/signup');
        }
        const newUser = new User({name, lastname, email, password});
        newUser.password = await  newUser.encryptPassword(password);
        await newUser.save();
        req.flash('success_msg', 'Tu usuario ha sido registrado');
